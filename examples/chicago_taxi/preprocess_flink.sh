...
DEFAULT_JOB_ID="chicago-taxi-preprocess-$(date +%Y%m%d-%H%M%S)"
JOB_ID="${JOB_ID:-$DEFAULT_JOB_ID}"
JOB_INPUT_PATH=$MYBUCKET/input/chicago_taxi_input
...
# move data to gcs
echo Uploading data to GCS if not uploaded
gsutil -q stat $JOB_INPUT_PATH/eval/* || gsutil cp -r ./data/eval/ ./data/train/ $JOB_INPUT_PATH/
